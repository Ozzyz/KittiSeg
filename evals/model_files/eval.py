...
def eval_image(hypes, gt_image, cnn_image):
    road = hypes["data"]["road_color"]
    background = hypes["data"]["background_color"]
    """."""
    pixel_max = max(road)
    thresh = np.array(range(0, pixel_max+1))/float(pixel_max) # Changed from 255
    road_gt = np.all(gt_image == road, axis=-1)
    background_gt = np.all(gt_image == background, axis=-1)
    valid_gt = road_gt + background_gt
...
def evaluate(hypes, sess, image_pl, inf_out):
    ...
    pixel_max = max(hypes["data"]["road_color"])
    thresh = np.array(range(0, pixel_max+1))/float(pixel_max)
